function drawText(textStr, sizeId, textX, textY, scale)
{
	...
	ctx.canvas.height = 110*scale;
	ctx.canvas.width = 450*scale;

	ctx.font = "bold "+(textSize[sizeId])*scale+"px Myriad Pro";
	...
	ctx.fillStyle = gradient;
	ctx.fillText(textStr, textX*scale, textY*scale);

	ctx.shadowColor = "rgba(0, 0, 0, 0)";
	ctx.strokeStyle = "#FFFFFF";
	ctx.lineWidth = lineSize[sizeId];
	ctx.strokeText(textStr, textX*scale, textY*scale);
}